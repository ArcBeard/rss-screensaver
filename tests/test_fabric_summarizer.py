"""Tests for FabricProcessor — extract_summary, caching, disabled mode, resource mgmt."""
...
from rss_screensaver import FabricProcessor, Headline
...
- Point one
- Point two"""
        result = FabricProcessor._extract_summary(output)
...
class TestSummarizerDisabled:
    def test_disabled_when_fabric_not_found(self):
        with patch("shutil.which", return_value=None):
            s = FabricProcessor(summary_enabled=True)
            assert s.enabled is False

    def test_disabled_by_config(self):
        with patch("shutil.which", return_value="/usr/bin/fabric"):
            s = FabricProcessor(summary_enabled=False, relevance_enabled=False)
            assert s.enabled is False
...
class TestSummarizerModel:
    def test_passes_model_flag_to_fabric(self):
        with patch("shutil.which", return_value="/usr/bin/fabric"):
            s = FabricProcessor(model="qwen3:4b")
...
class TestProcessNextPage:
    def test_summarizes_only_page_size_headlines(self):
        with patch("shutil.which", return_value="/usr/bin/fabric"):
            s = FabricProcessor(model="qwen3:4b", relevance_enabled=False)
        ...
        with patch("subprocess.run", side_effect=[ps_result, mock_result, mock_result, mock_result]):
            s.process_next_page(headlines, 3)
    ...
    def test_skips_when_ollama_busy(self):
        ...
        with patch("subprocess.run", return_value=ps_result) as mock_run:
            s.process_next_page(headlines, 1)
    ...
    def test_skips_already_summarized(self):
        ...
        with patch("subprocess.run", return_value=ps_result) as mock_run:
            s.process_next_page([h], 1)
            assert mock_run.call_count == 1

    def test_scores_relevance_when_enabled(self):
        with patch("shutil.which", return_value="/usr/bin/fabric"):
            s = FabricProcessor(model="qwen3:4b", summary_enabled=False, relevance_enabled=True)

        ps_result = MagicMock()
        ps_result.returncode = 0
        ps_result.stdout = "NAME    ID    SIZE    PROCESSOR    CONTEXT    UNTIL\n"

        relevance_result = MagicMock()
        relevance_result.returncode = 0
        relevance_result.stdout = '{"score": 8, "reason": "Relevant to AI interests."}'

        h = Headline("AI Agents", "S", link="http://x.com", description="Text about AI")

        with patch("subprocess.run", side_effect=[ps_result, relevance_result]):
            s.process_next_page([h], 1)

        assert h.relevance_score == 8
        assert "AI" in h.relevance_reason
